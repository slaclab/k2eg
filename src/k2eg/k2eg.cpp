...
K2EGateway::K2EGateway()
    : po(std::make_unique<k2eg::common::ProgramOptions>())
    , quit(false)
    , terminated(false) {}
...
int K2EGateway::setup(int argc, const char* argv[]) {
    ...
    try {
        ...
        ServiceResolver<IPublisher>::registerService(
            std::make_shared<RDKafkaPublisher>(po->getPublisherConfiguration()));
        logger->logMessage("Start subscriber service");
        ServiceResolver<ISubscriber>::registerService(
            std::make_shared<RDKafkaSubscriber>(po->getSubscriberConfiguration()));
        ...
        cmd_controller = std::make_unique<CMDController>(
            po->getCMDControllerConfiguration(), std::bind(&K2EGateway::commandReceived, this, std::placeholders::_1));
...
const std::string K2EGateway::getTextVersion(bool long_version) {
    return long_version?
STRING_FORMAT(R"VERSION(k2eg Epics Kafka Gateway %1%.%2%.%3%)VERSION",
    k2eg_VERSION_MAJOR%k2eg_VERSION_MINOR%k2eg_VERSION_PATCH):
...
SqliteORM               %8%
    ...
    k2eg_VERSION_MAJOR%k2eg_VERSION_MINOR%k2eg_VERSION_PATCH%k2eg_BOOST_VERSION%k2eg_EPICS_VERSION%k2eg_LIBLZ4_VERSION%k2eg_SQLITE_VERSION%k2eg_SQLITEORM_VERSION)   
    ;
}