...
void NodeConfiguration::loadNodeConfiguration()
{
    ...
    {
        auto data_insert_res =
            channel_repository->insert(ChannelMonitorType{.pv_name = desc.first,
...
std::vector<bool> NodeConfiguration::addChannelMonitor(const ChannelMonitorTypeConstVector& channel_descriptions)
{
    ...
    for (auto const& desc : channel_descriptions)
    {
        ...
        auto new_conf_for_pv = sc::MakePVMonitorInfoShrdPtr(sc::PVMonitorInfo{.pv_destination_topic = desc.channel_destination, .event_serialization = static_cast<std::uint8_t>(desc.event_serialization)});
...
size_t NodeConfiguration::iterateAllChannelMonitor(size_t element_to_process, ChannelMonitorTypeHandler handle)
{
    ...
    for (auto& ch : distinct_name_prot)
    {
        ...
        {
            if (++processed > element_to_process)
                break;
            int dummy_flag = 0;
...
size_t NodeConfiguration::iterateAllChannelMonitorForAction(size_t element_to_process, ChannelMonitorTypeHandler purge_handler)
{
    auto   channel_repository = toShared(data_storage->getChannelRepository());
    auto   distinct_name_prot = channel_repository->getDistinctByNameProtocol();
    size_t processed = 0;
...
const std::string NodeConfiguration::getNodeName() const
{
    if (node_configuration_service)
    {
        return node_configuration_service->getNodeName();
    }
    else
    {
        return "";
