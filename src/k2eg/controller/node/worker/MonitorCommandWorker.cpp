...
MonitorCommandWorker::MonitorCommandWorker(const MonitorCommandConfiguration& monitor_command_configuration, EpicsServiceManagerShrdPtr epics_service_manager, NodeConfigurationShrdPtr node_configuration_db)
    ...
    , run_rate_thread(false)
{
    ...
    auto task_restart_monitor = MakeTaskShrdPtr(STARTUP_MONITOR_TASK_NAME, STARTUP_MONITOR_TASK_NAME_CRON, std::bind(&MonitorCommandWorker::handleRestartMonitorTask, this, std::placeholders::_1),
                                                -1 // start at application boot time
    );
    ServiceResolver<Scheduler>::resolve()->addTask(task_restart_monitor);

    publisher->setCallBackForReqType("monitor-message", std::bind(&MonitorCommandWorker::publishEvtCB, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
}
...
void MonitorCommandWorker::executePeriodicTask()
{
    TaskProperties task_properties;
    handlePeriodicTask(task_properties);
}

void MonitorCommandWorker::publishEvtCB(pubsub::EventType type, PublishMessage* const msg, const std::string& error_message)
{
    switch (type)
    {
...
void MonitorCommandWorker::handleMonitorCheckEvents(MonitorHandlerData checker_event_data)
{
    ...
    case MonitorHandlerAction::Start:
        {
            ...
            // got start event
            if (std::find_if(std::begin(pv_monitor_info.cmd_vec), std::end(pv_monitor_info.cmd_vec),
                             [&checker_event_data](auto& info_topic)
                             {
            ...
            else
            {
                ...
                logger->logMessage(
                    STRING_FORMAT("Monitor for '%1%' for topic '%2%' already activated",
                                  checker_event_data.monitor_type.pv_name % checker_event_data.monitor_type.channel_destination));
            }
            break;
        }
    ...
    case MonitorHandlerAction::Stop:
        {
            ...
            logger->logMessage(STRING_FORMAT("Stop monitor on '%1%' for topic '%2%'",
                                             checker_event_data.monitor_type.pv_name % checker_event_data.monitor_type.channel_destination));
            auto itr = std::find_if(std::begin(pv_monitor_info.cmd_vec), std::end(pv_monitor_info.cmd_vec),
                                    [&checker_event_data](auto& info_topic)
                                    {
                                        return info_topic.channel_destination.compare(checker_event_data.monitor_type.channel_destination) == 0;
                                    });
            if (itr != std::end(pv_monitor_info.cmd_vec))
            {
                ...
                logger->logMessage(
                    STRING_FORMAT("Monitor stopped on '%1%' for topic '%2%'",
                                  checker_event_data.monitor_type.pv_name % checker_event_data.monitor_type.channel_destination));
            ...
            {
                logger->logMessage(
                    STRING_FORMAT("No active monitor on '%1%' for topic '%2%'",
                                  checker_event_data.monitor_type.pv_name % checker_event_data.monitor_type.channel_destination));
            }
            break;
        }
...
void MonitorCommandWorker::processCommand(std::shared_ptr<BS::light_thread_pool> thread_pool, ConstCommandShrdPtr command)
{
    if (starting_up)
    {
...
void MonitorCommandWorker::manage_single_monitor(k2eg::controller::command::cmd::ConstCommandShrdPtr command)
{
    bool activate = false;
    ...
    monitor_checker_shrd_ptr->storeMonitorData(
        {ChannelMonitorType{.pv_name = cmd_ptr->pv_name,
...
const std::string MonitorCommandWorker::get_queue_for_pv(const std::string& pv_name)
{
    return std::regex_replace(pv_name, std::regex(":"), "_");
}

void MonitorCommandWorker::manage_multiple_monitor(k2eg::controller::command::cmd::ConstCommandShrdPtr command)
{
    bool                            activate = false;
    auto                            cmd_ptr = static_pointer_cast<const MultiMonitorCommand>(command);
    std::vector<ChannelMonitorType> monitor_command_vec;
    std::ranges::for_each(
        cmd_ptr->pv_name_list,
        [&cmd_ptr, &monitor_command_vec, this](const std::string& pv_name)
        {
...
bool MonitorCommandWorker::isReady()
{
    return !starting_up;
}

void MonitorCommandWorker::manageReply(const std::int8_t error_code, const std::string& error_message, ConstCommandShrdPtr cmd)
{
    ...
    for (auto& event : *event_received->event_data)
    {
        ...
        for (auto& monitor_info : channel_topics_map[event->channel_data.pv_name].cmd_vec)
        {
            ...
            publisher->pushMessage(
                MakeReplyPushableMessageUPtr(monitor_info.channel_destination, "monitor-message", event->channel_data.pv_name,
                                             local_serialization_cache[static_cast<SerializationType>(monitor_info.event_serialization)]),
                {// add header
