...
class SnapshotCommandWorker : public CommandWorker {
  ...
  void checkGetCompletion(std::shared_ptr<BS::priority_thread_pool> thread_pool, SnapshotOpInfoShrdPtr snapshot_info);
 ...
 public:
  ...
  // process the command
  void processCommand(std::shared_ptr<BS::priority_thread_pool> thread_pool, k2eg::controller::command::cmd::ConstCommandShrdPtr command);
};
