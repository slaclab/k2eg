...
MessageSerType MapToCommand::getSerializationType(const boost::json::value& v) {
        ...
        if (auto activation = obj.if_contains(KEY_ACTIVATE); activation != nullptr && activation->is_bool()) {
            if (activation->as_bool()) {
                if (auto fields = checkFields(obj, {{KEY_PROTOCOL, kind::string}, {KEY_CHANNEL_NAME, kind::string}, {KEY_DEST_TOPIC, kind::string}}); fields != nullptr) {
                    ...
                    if (auto v = obj.if_contains(KEY_SERIALIZATION)) {
                        ser_type = getSerializationType(*v);
                    }
                ...
                if (auto fields = checkFields(obj, {{KEY_CHANNEL_NAME, kind::string}, {KEY_DEST_TOPIC, kind::string}}); fields != nullptr) {
                    result = std::make_shared<MonitorCommand>(
                        MonitorCommand{CommandType::monitor, MessageSerType::unknown, "", std::any_cast<std::string>(fields->find(KEY_CHANNEL_NAME)->second), false, std::any_cast<std::string>(fields->find(KEY_DEST_TOPIC)->second)});
                } else {
                    logger->logMessage("Missing key for the AquireCommand: " + serialize(obj), LogLevel::ERROR);
                }
            }
    ...
    case CommandType::put: {
        if (auto fields = checkFields(obj, {{KEY_PROTOCOL, kind::string}, {KEY_CHANNEL_NAME, kind::string}, {KEY_VALUE, kind::string}}); fields != nullptr) {
            ...
            result = std::make_shared<PutCommand>(PutCommand{
                CommandType::put, ser_type, std::any_cast<std::string>(fields->find(KEY_PROTOCOL)->second), std::any_cast<std::string>(fields->find(KEY_CHANNEL_NAME)->second), std::any_cast<std::string>(fields->find(KEY_VALUE)->second)});
        } else {
