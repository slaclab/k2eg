    ...
    : ILogger(std::move(_configuration))
{
    ...
    {
        console_sink = logging::add_console_log(std::clog, logging::keywords::format = EXTENDED_LOG_FORMAT);
    }
    ...
    if (configuration->log_on_file)
    {
        file_sink = logging::add_file_log(keywords::file_name = configuration->log_file_name,                                    // file name pattern
                                          ...
                                          keywords::format = EXTENDED_LOG_FORMAT,
    ...
    if (configuration->log_on_syslog)
    {
        ...
        syslog_sink.reset(
            new sinks::synchronous_sink<sinks::syslog_backend>(keywords::use_impl = sinks::syslog::udp_socket_based, keywords::format = EXTENDED_LOG_FORMAT));
...
{
    logger.logMessage(std::format("[{}] {}", scope, message), level);
}