...
BoostLogger::BoostLogger(ConstLogConfigurationUPtr _configuration)
    ...
    if (configuration->log_on_file) {
        file_sink = logging::add_file_log(
            keywords::file_name = configuration->log_file_name,                          // file name pattern
    ...
    if (configuration->log_on_syslog) {
        ...
        syslog_sink.reset(new sinks::synchronous_sink<sinks::syslog_backend>(
            keywords::use_impl = sinks::syslog::udp_socket_based, keywords::format = EXTENDEND_LOG_FORMAT));
        // Setting the remote address to sent syslog messages to.
        syslog_sink->locked_backend()->set_target_address(configuration->log_syslog_srv,
                                                          configuration->log_syslog_srv_port);
    ...
    logger->set_logging_enabled(configuration->log_on_console || configuration->log_on_file
