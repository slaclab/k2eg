...
PrometheusEpicsMetric::PrometheusEpicsMetric()
    ...
    , idle_counter_family(
          BuildCounter().Name("k2eg_thread_idle_cycles_total").Help("Total number of idle cycles per thread").Register(*registry))
    , event_counter_family(
          BuildCounter().Name("k2eg_thread_events_processed_total").Help("Total number of events processed per thread").Register(*registry))
    , duration_counter_family(prometheus::BuildCounter()
                                  .Name("k2eg_thread_poll_duration_microseconds")
                                  .Help("Total poll time in microseconds per thread")
                                  .Register(*registry))
    , throttle_gauge_family(BuildGauge().Name("k2eg_thread_throttle_ms").Help("Current throttle delay per thread in ms").Register(*registry))
