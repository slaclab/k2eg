...
void
PVStructureMerger::appendFieldFromStruct(std::vector<pvd::PVStructure::const_shared_pointer> struct_ptr_vec, bool reset) {
  if (!struct_ptr_vec.size()) return;
  if (reset) { root_builder = pvd::getFieldCreate()->createFieldBuilder(); }
...
void
PVStructureMerger::copyStructure(epics::pvData::FieldBuilderPtr builder, const epics::pvData::PVStructure* struct_ptr) {
...
void
PVStructureMerger::copyValue(epics::pvData::PVStructure* dest_structure, const pvd::PVStructure* src_structure) {
