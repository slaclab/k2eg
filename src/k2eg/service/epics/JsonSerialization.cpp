...
void JsonSerializer::serialize(const ChannelData& message, SerializedMessage& serialized_message)
{
...
SerializedMessageShrdPtr JsonSerializer::serialize(const ChannelData& message, const std::string& reply_id)
{
    ...
    if (!reply_id.empty())
    {
        json_message->getJsonObject()[KEY_REPLY_ID] = reply_id;
    }
    return json_message;
}
...
void JsonSerializer::processField(const epics::pvData::PVField* field, const std::string& key, boost::json::object& json_object)
{
        ...
        case pvd::Type::union_:
            {
                ...
                if (field)
                {
                    processField(field.get(), key, json_object);
                }
                break;
            }
...
void JsonSerializer::processStructureArray(pvd::PVStructureArray::const_svector structure_array, const std::string& key, boost::json::object& json_object)
{
