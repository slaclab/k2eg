...
inline PVMonitorType toPVMonitor(const k2eg::controller::command::cmd::MonitorCommand& acquire_command) {
    return PVMonitorType {
...
inline  k2eg::controller::command::cmd::ConstCommandShrdPtr  fromPVMonitor(const PVMonitorType& command) {
...
public:
    ...
    void remove(const PVMonitorType& pv_description);
    bool isPresent(const PVMonitorType& new_cannel) const;
    std::optional<PVMonitorTypeUPtr> getPVMonitor(const PVMonitorType& pv_description) const;
    PVMonitorDistinctResultType getDistinctByNameProtocol() const;
    void processAllPVMonitor(const std::string& pv_name,
